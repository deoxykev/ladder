...
// to the client.
func BlockIncomingCookies(whitelist ...string) proxychain.ResMod {
	return func(px *proxychain.ProxyChain) error {
		px.Resp.Header.Del("Set-Cookie")
		return nil
	}
}
...
func BlockIncomingCookiesExcept(whitelist ...string) proxychain.ResMod {
	return func(px *proxychain.ProxyChain) error {
		...
		if px.Resp.Header == nil {
			return nil
		}

		// Filter the cookies in the response
		filteredCookies := []string{}
		for _, cookieStr := range px.Resp.Header["Set-Cookie"] {
		...
		if len(filteredCookies) > 0 {
			px.Resp.Header["Set-Cookie"] = filteredCookies
		} else {
			px.Resp.Header.Del("Set-Cookie")
		}
